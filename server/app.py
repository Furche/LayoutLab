"""Stdlib HTTP service for headless Room Model commands (DD-014 Phase B).
...
# Repo root on sys.path so `layoutlab` imports work when run as `python -m server`.
...
class Handler(BaseHTTPRequestHandler):
    server_version = "LayoutLabCore/0.1"
    ...
    def do_POST(self):
        path = urlparse(self.path).path
        if path not in ("/v1/commands", "/v1/commands/"):
            self._json(404, {"ok": False, "error": f"not found: {path}"})
            return
