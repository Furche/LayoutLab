...
try:
    import bpy
except ImportError:  # Headless Core / local server (DD-014 Phase B)
    bpy = None
...
if bpy is not None:
    ...
    def register():
        ...
        bpy.types.Scene.layoutlab_command_source = bpy.props.EnumProperty(
            name="Command Source",
            items=[
                ("CLIPBOARD", "Clipboard", "Read JSON commands from clipboard"),
                ("TEXT", "Text Block", "Read JSON commands from a Blender text block"),
            ],
            default="CLIPBOARD",
        )
        bpy.types.Scene.layoutlab_text_block_name = bpy.props.StringProperty(
            name="Command Text Block", default="LayoutLab_Commands"
        )
        bpy.types.Scene.layoutlab_generator_text_block_name = bpy.props.StringProperty(
            name="Generator Text Block", default="LayoutLab_Generator_Code"
        )
        bpy.types.Scene.layoutlab_selected_generator = bpy.props.StringProperty(
            name="Selected Generator", default="bed_basic"
        )
        bpy.types.Scene.layoutlab_generator_items = bpy.props.CollectionProperty(
            type=LayoutLabGeneratorItem
        )
        bpy.types.Scene.layoutlab_generator_index = bpy.props.IntProperty(
            default=0, update=_on_generator_index_changed
        )
        ...
        bpy.types.Scene.layoutlab_quick_test_profile_gen = bpy.props.StringProperty(
            name="Quick Test Profile", default=""
        )
        bpy.types.Scene.layoutlab_test_object_name = bpy.props.StringProperty(
            name="Object Name", default="TEST_BED"
        )
        bpy.types.Scene.layoutlab_test_location = bpy.props.FloatVectorProperty(
            name="Location", size=3, default=(0.0, 0.0, 0.0)
        )
    ...
    def register():
        raise RuntimeError("LayoutLab register() requires Blender (bpy)")

    def unregister():
        raise RuntimeError("LayoutLab unregister() requires Blender (bpy)")
